            ...
            @foreach($categorys as $category)
                ...
                @if(isset($category->allChilds)&&!$category->allChilds->isEmpty())
                    @foreach($category->allChilds as $child)
                        <option value="{{ $child->id }}" @if(isset($product->category_id)&&$product->category_id==$child->id)selected @endif >&nbsp;&nbsp;&nbsp;┗━━{{ $child->name }}</option>
                        @if(isset($child->allChilds)&&!$child->allChilds->isEmpty())
                            @foreach($child->allChilds as $third)
                                <option value="{{ $third->id }}" @if(isset($product->category_id)&&$product->category_id==$third->id)selected @endif >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;┗━━{{ $third->name }}</option>
                            @endforeach
                        @endif
                    @endforeach
                @endif
...
<div class="layui-form-item">
    ...
    <div class="layui-input-block">
        <input type="text" name="keywords" value="{{$product->keywords??old('keywords')}}" lay-verify="required" placeholder="请输入关键词" class="layui-input" >
    </div>
</div>
...
<div class="layui-form-item">
    ...
    <div class="layui-input-block">
        <input type="number" name="click" value="{{$product->click??mt_rand(100,500)}}" lay-verify="required|numeric"  class="layui-input" >
    </div>
</div>
            ...
            <div class="layui-upload-list" >
                <ul id="layui-upload-box" class="layui-clear">
                    @if(isset($product->thumb))
                        <li><img src="{{ $product->thumb }}" /><p>上传成功</p></li>
                    @endif
                </ul>
