    ...
    <div class="layui-card">
        <div class="layui-card-header layuiadmin-card-header-auto">
            <div class="layui-btn-group">
                    <a class="layui-btn layui-btn-sm" href="{{ route('admin.product_category.create') }}">添 加</a>
        ...
        <script>
            layui.use(['layer','table','form'],function () {
                ...
                var dataTable = table.render({
                    elem: '#dataTable'
                    ,height: 500
                    ,url: "{{ route('admin.product_category.data') }}" //数据接口
                    ,page: true //开启分页
                    ,cols: [[ //表头
                ...
                table.on('tool(dataTable)', function(obj){ //注：tool是工具条事件名，dataTable是table原始容器的属性 lay-filter="对应的值"
                    ...
                    if(layEvent === 'del'){
                        layer.confirm('确认删除吗？', function(index){
                            $.post("{{ route('admin.product_category.destroy') }}",{_method:'delete',ids:data.id},function (result) {
                                if (result.code==0){
                        ...
                        location.href = '/admin/product_category/'+data.id+'/edit';
