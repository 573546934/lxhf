	...
	Vue.component("vue-slideshow", {
		...
		methods: {
			...
			dotChange: function(index, useCustomDot) {
				...
				const $dot = typeof useCustomDot === "string" ? [].slice.call($(useCustomDot).children) : $$(
					".v-slideshow-dot i", this.$el);
		...
		mounted: function() {
			...
			for (let temp of list) {
				...
				newImg.onload = () => { // 图片加载成功后把地址给原来的img
					...
					temp.show=true
					// temp.setAttribute('display', 'block');
				}
			}
			this.data = [...list]
